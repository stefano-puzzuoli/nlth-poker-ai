...
def main():
    ...
    for i in range(5):
        ...
        name = 'Player ' + str(i+1)
        p = Player(name=name, reg=r, bankroll=10**6,
                   n_raises=1000, r_factor=0.7, memory=10**5)
        players.append(p)

    r = GradientBoostingRegressor()
    name = 'Player ' + str(i+1)
    p = Player(name="User", reg=r, bankroll=10**6,
               n_raises=1000, r_factor=0.7, memory=10**5)
    players.append(p)
    ...
    if ai_difficulty == 1:
        ...
        simulate(t, n_hands=100000, first_train=2000,
                 n_train=1000, n_buy_in=10)
    else:
        simulate(t, n_hands=100000, first_train=2000,
                 n_train=1000, n_buy_in=10)
